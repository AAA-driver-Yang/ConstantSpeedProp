...
namespace ConstantSpeedProp
{
    ...
    ///
    /// 0.4.5 CN UI: all fields visible. Filtered Forward Lift Guard restored for negative forward-lift recovery.
    ///
    /// 0.4.0 design goal:
    ...
    {
        private const string VersionText = "0.4.1 中文版";
        ...
        [KSPField(isPersistant = true, guiActive = false, guiActiveEditor = false, guiName = "高级选项")]
        ...
        [KSPField(isPersistant = false, guiActive = true, guiActiveEditor = true, guiName = "叶片速度", guiUnits = " m/s", guiFormat = "F1")]
        public float measuredBladeSpeedDisplay;

        [KSPField(isPersistant = false, guiActive = true, guiActiveEditor = true, guiName = "轴向空速", guiUnits = " m/s", guiFormat = "F1")]
        ...
        private static readonly string[] compactVisibleFields =
        {
            ...
            "forwardLiftGuardMinThrottlePercent",
        ...
        {
            // 0.4.5: folded PAW removed. KSP editor PAW does not always refresh dynamic visibility reliably,
            // so all normal, advanced, and diagnostic fields are kept visible in both editor and flight.
            ...
            // Keep the old persisted flag harmless but remove the actual UI switch.
            SetFieldVisible("showAdvancedOptions", false);
        ...
        private string LocalizeAoaSource(string source)
        {
            ...
            text = text.Replace("DLC Lift Only", "仅DLC升力");
        ...
        private void RunGovernorLoop(string source, float dt)
        {
            ...
            forwardLiftGuardActiveDisplay = IsForwardLiftGuardArmed() ? "是" : "否";
        ...
        {
            if (!raw.valid && !raw.hasForwardLift)
                return raw;
        ...
        {
            // 0.4.5: filtered negative forward-lift guard.
            // When forward lift turns negative, do not allow the governor to keep driving blade pitch finer.
            // Clamp the command toward a positive/coarser recovery rate, then skip the AoA branch for this frame.
            if (IsForwardLiftGuardArmed() && aero.hasForwardLift && aero.minForwardLift < -Mathf.Max(0f, negativeForwardLiftDeadband))
            {
                float deficit = -aero.minForwardLift - Mathf.Max(0f, negativeForwardLiftDeadband);
                float severity = Mathf.Clamp01(deficit / 10f + 0.25f);
                float recoveryRate = Mathf.Abs(negativeLiftRecoveryRate) * severity;
                requestedPitchRate = Mathf.Max(requestedPitchRate, recoveryRate);
                integralError = 0f;
                return;
            }
        ...
        private AeroData GetAeroData(float wantedRpm)
        {
            ...
            if (useDlcAeroData)
            {
                ...
                if (dlcAoaSamples > 0 || samples > 0 || liftSamples > 0)
                {
                    ...
                    else
                    {
                        ...
                        result.source = samples > 0 ? "Vector AoA" : "DLC Lift Only";
                    }
            ...
            result.hasForwardLift = false;
            result.averageForwardLift = 0f;
            result.minForwardLift = 0f;
            result.forwardLiftSampleCount = 0;
